...
from utils import relative_to_abs, get_dset_path, get_log
...
# Output
parser.add_argument('--output_dir', default=os.path.join(sys.path[0], 'checkpoints'))
...
logger = get_log(os.path.join(sys.path[0],'logs', args.checkpoint_name+'.txt'), __file__.split('/')[0])
...
def main(args):
    # record losses in the csv
    f_train = open(os.path.join(sys.path[0],'logs', args.checkpoint_name+'_train.csv'), 'w')
    header_train = ['epoch', 'D_data_loss', 'D_total_loss', 'G_discriminator_loss', 'G_l2_loss_rel', 'G_total_loss']
    writer_train = csv.writer(f_train)
    writer_train.writerow(header_train)

    f_eval = open(os.path.join(sys.path[0],'logs', args.checkpoint_name+'_eval.csv'), 'w')
    header_eval = ['epoch', 'Val_ade', 'Val_ade_l', 'Val_ade_nl', 'Val_d_loss', 'Val_fde', 'Val_fde_l', 'Val_fde_nl', 'Val_g_l2_loss_abs', 'Val_g_l2_loss_rel',
                    'Train_ade', 'Train_ade_l', 'Train_ade_nl', 'Train_d_loss', 'Train_fde', 'Train_fde_l', 'Train_fde_nl', 'Train_g_l2_loss_abs', 'Train_g_l2_loss_rel']
    ...
    _, val_loader = data_loader(args, test_path)
    ...
    if restore_path is not None and os.path.isfile(restore_path):
        ...
        checkpoint['metrics_val'] = defaultdict(list)
        ...
        for batch in train_loader:
            ...
            if t % args.print_every == 0:
                ...
                for k, v in sorted(losses_d.items()):
                    ...
                    train_row.append('{:.04f}'.format(v))
                for k, v in sorted(losses_g.items()):
                    logger.info('  [G] {}: {:.3f}'.format(k, v))
                    checkpoint['G_losses'][k].append(v)
                    train_row.append('{:.04f}'.format(v))
                checkpoint['losses_ts'].append(t)
                writer_train.writerow(train_row)
            ...
            if t > 0 and t % args.checkpoint_every == 0:
                ...
                for k, v in sorted(metrics_val.items()):
                    ...
                    eval_row.append('{:.04f}'.format(v))
                for k, v in sorted(metrics_train.items()):
                    logger.info('  [train] {}: {:.3f}'.format(k, v))
                    checkpoint['metrics_train'][k].append(v)
                    eval_row.append('{:.04f}'.format(v))
                writer_eval.writerow(eval_row)
                ...
                checkpoint_path = os.path.join(
                    args.output_dir, '{}_{}_with_model.pt'.format(args.checkpoint_name, t)
                )
                logger.info('Saving checkpoint to {}'.format(checkpoint_path))
                torch.save(checkpoint, checkpoint_path)

                if metrics_val['ade'] == min_ade:
                    best_checkpoint_path = os.path.join(args.output_dir, '{}_{}_best_with_model.pt'.format(args.checkpoint_name, t))
                ...
                checkpoint_path = os.path.join(
                    args.output_dir, '{}_{}_no_model.pt'.format(args.checkpoint_name, t))
                logger.info('Saving checkpoint to {}'.format(checkpoint_path))
                key_blacklist = [
